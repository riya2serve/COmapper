...
ver1.3.0(241219): change input, output file format to include QNAME column in SAM file
...
class Result:
    def __init__(self, qname, chr_num, pos, num_of_snp, snp_pos, bases, types, SNPs, length):
        self.qname = qname
        self.pos = pos
    ...
    def print_result(self):
        print(self.qname, "\t", self.chr_num, "\t", self.pos, "\t", self.length, "\t", self.num_of_snp, "\t", self.snp_pos, "\t",
              self.bases, "\t", self.types, "\t", self.SNPs, "\t", self.haplotype, "\t", self.CO_info)

    def write_result(self, df):
        a = ([[self.qname, self.chr_num, self.pos, self.length, self.num_of_snp, self.snp_pos, self.bases, self.types, self.SNPs, self.haplotype, self.CO_info]])
        df = df.append(a)
        return df
...
def CO_detect_file (pos_list, col_list, ler_list, file):
    dt = {'qname':str, 'chr':str, 'pos':int, 'cigar':str, 'seq':str}
    f = pd.read_csv(file, delimiter='\t', names=['qname', 'chr', 'pos', 'cigar', 'seq'], header=None, dtype=dt, comment='q', error_bad_lines=False)
        ...
        for i, read in sam_list[chr_cnt].iterrows():  # Take each sequence
            ...
            for j in range(start_idx, num_of_snp):
                ...
                if 0 <= snp_position - start_pos < length:
                    ...
                    break
            list_of_results.append(Result(read['qname'], chr_cnt + 1, start_pos, snp_cnt, pos_list_temp, allele_temp, genotype_temp, SNP_temp, length))
            read_cnt += 1
...
def main():
    ...
    try:
        ...
        parser.add_argument('--snpfile', '-s', help='snp file location, default: collerF2.masked.tiger.txt', default=os.getcwd()+'/resources/coller_marker_v4.6.txt')
        parser.add_argument('--threads', '-t', help = 'thread number, default: 1', default = 1)
    ...
    # create output file
    if(df.empty==False):
        df.columns=(['qname', 'chr_num', 'pos', 'length', 'num_of_snp','snp_pos', 'bases', 'types', 'SNP_genotype', 'haplotype', 'CO_info'])
